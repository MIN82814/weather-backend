...
function formatWeather(locationData) {
  ...
  const Wx = locationData.weatherElement.find((e) => e.elementName === "Wx"); // 天氣
  ...
  const times = Wx.time; // 取 Wx 的時段作為主迴圈
...
const getWeatherByLocation = async (req, res) => {
  try {
    ...
    // city
    if (city) {
      cityName = normalizeCity(city.trim());
    }
...
app.get("/", (req, res) => {
  res.json({
    message: "Weather API",
    example: "/api/weather?city=臺北市 或 /api/weather?lat=25.03&lng=121.56",
  });
});
